...
//
//----------------------------------------------------------------------------------
// Some modifications have been performed in the code for use in ciam project
//----------------------------------------------------------------------------------
// Modified by: Bastian Harder
// - in function Rhd2000Registers::createCommandListRegisterConfig(...)
//    -> added 9 dummy commands after calibration command to comply with the datasheet
//    -> commented out configuration of amplifiers 32-63 as they are not used
// - in constructor:
//	  -> disabled zcheckDacPower
//	  -> defined variable digOut
//
//----------------------------------------------------------------------------------
...
Rhd2000Registers::Rhd2000Registers(double sampleRate)
{
	...
	//Modification: set digOut = 0 to make the variable defined. The output is not used though...
	digOut = 0;					//actually not used
	...
	// Modification: Set zcheckDacPower = 0 instead of 1
...
// Create a list of 67 (originally 60) commands to program most RAM registers on a RHD2000 chip, read those values
...
int Rhd2000Registers::createCommandListRegisterConfig(vector<uint16_t> &commandList, bool calibrate)
{
	...
	// Modification: as no Temp Sensor or aux output/input is used, Register 3 is written as well
	...
	// Modification: added two dummy commands here to be able to read the result of RegRead 17
	commandList.push_back(createRhd2000Command(Rhd2000CommandRegRead, 63));
	commandList.push_back(createRhd2000Command(Rhd2000CommandRegRead, 63));
    ...
    if (calibrate) {
		...
		// Modification: added 9 dummy commands here
		commandList.push_back(createRhd2000Command(Rhd2000CommandRegRead, 63));
		commandList.push_back(createRhd2000Command(Rhd2000CommandRegRead, 63));
		commandList.push_back(createRhd2000Command(Rhd2000CommandRegRead, 63));
		commandList.push_back(createRhd2000Command(Rhd2000CommandRegRead, 63));
		...
		// add dummy commands to produce a command list of equal length in case of no calibration
        commandList.push_back(createRhd2000Command(Rhd2000CommandRegRead, 63));
		commandList.push_back(createRhd2000Command(Rhd2000CommandRegRead, 63));
		commandList.push_back(createRhd2000Command(Rhd2000CommandRegRead, 63));
		commandList.push_back(createRhd2000Command(Rhd2000CommandRegRead, 63));
	...
	// Modification: commented out the following 4 lines as we only have 16 amplifiers
	...
	// Modification: commented out because the command is considered useless
    // commandList.push_back(createRhd2000Command(Rhd2000CommandRegRead, 63));
...
// Create a list of 18 commands to convert data from all 16 channel
// Returns the length of the command list.
