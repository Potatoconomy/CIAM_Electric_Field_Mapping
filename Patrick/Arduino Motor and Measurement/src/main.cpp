...
//adsGain_t m_gain = GAIN_ONE;  // long term solution will actively determine Gain setting  
// note wrong value is getting printed right now
const int GAIN = 8;
//const char *gain_names[] = { "GAIN_TWOTHIRDS", "GAIN_ONE", "GAIN_TWO", "GAIN_FOUR", "GAIN_EIGHT", "GAIN_SIXTEEN"};



bool output = 0;  
//int8_t gain = ads1115.getGain()
...
void loop() {
  // Cannot work with relay mode currently.
  set_motor_mode();
}
...
void print_motor_values(long position, unsigned long time) {
...
void get_gain() {
  switch (GAIN) {
    case 0:  ads1115.setGain(GAIN_TWOTHIRDS); break; // 2/3x gain +/- 6.144V  LSB = 187.5  uV 
    case 1:  ads1115.setGain(GAIN_ONE);       break; // 1x gain   +/- 4.096V  LSB = 125    uV
    case 2:  ads1115.setGain(GAIN_TWO);       break; // 2x gain   +/- 2.048V  LSB = 62.5   uV
    case 4:  ads1115.setGain(GAIN_FOUR);      break; // 4x gain   +/- 1.024V  LSB = 31.25  uV
    case 8:  ads1115.setGain(GAIN_EIGHT);     break; // 8x gain   +/- 0.512V  LSB = 15.625 uV
    case 16: ads1115.setGain(GAIN_SIXTEEN);   break; // 16x gain  +/- 0.256V  LSB = 7.8125 uV
    default: ads1115.setGain(GAIN_ONE);       break;
  }
}
...
void perform_round_trip()
{
  ...
  // controls motor speed and direction while constantly measuring adc and encoder
  ...
  while(current_pos < 300)
  {
    ...
    print_motor_values(current_pos, millis()-myTime);
  }
  ...
  Serial.end();
  while(1){} // end of script. must reset arduino
}
